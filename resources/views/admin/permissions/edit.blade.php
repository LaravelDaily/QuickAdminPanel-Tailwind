...
<div class="max-w w-full bg-white shadow-md rounded-md overflow-hidden border">
    <div class="flex justify-between items-center px-5 py-3 text-gray-700 border-b">
        <h3 class="text-sm">{{ trans('global.edit') }} {{ trans('cruds.permission.title_singular') }}</h3>
    </div>
        ...
        <div class="px-5 py-6 bg-gray-200 text-gray-700 border-b">
            <div class="mb-3">
                ...
                <div class="mt-2 relative rounded-md shadow-sm">
                    <input type="text" id="title" name="title" class="form-input w-full px-3 py-2 appearance-none rounded-md focus:border-indigo-600{{ $errors->has('title') ? ' border-red-500' : '' }}" value="{{ old('title', $permission->title) }}" required>
                </div>
                @if($errors->has('title'))
